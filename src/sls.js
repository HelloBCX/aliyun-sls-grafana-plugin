...
function b64HMACSHA12Sign(key, payload) {
  const payloadArr = stringToUtf8ByteArray(payload);
  return _b64HMACSHA12Sign(
    key,
    payloadArr.map(i => String.fromCharCode(i)).join(''),
    '=',
    8
  );
}
...
export class SLS {
  constructor(config, backendSrv, baseUrl) {
  ...
  signRequest(requestMethod, url, logHeaders, body) {
    const bodyStr = this.serializeBody(body, v => v);
    const payload = [
      requestMethod,
      '\n\n\n',
  ...
  requestData(method, url, headers, body) {
    ...
    const logHeaders = this.getLogHeaders();
    const requestSignature = this.signRequest(
      requestMethod,
      url,
      logHeaders,
      body
